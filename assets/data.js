...
// Automatic CDN path resolver for Shopify theme environment
if (typeof window !== 'undefined') {
  function updateDataImagePaths() {
    if (window.ShopifyTheme && window.ShopifyTheme.assetUrl) {
      let raw = window.ShopifyTheme.assetUrl.split('?')[0];
      if (!raw.endsWith('/')) {
        raw = raw.substring(0, raw.lastIndexOf('/') + 1);
      }
      LUMORA_DATA.products.forEach(p => {
        if (p.image && p.image.indexOf('/') !== -1) {
          p.image = raw + p.image.split('/').pop();
        }
      });
      LUMORA_DATA.categories.forEach(c => {
        if (c.image && c.image.indexOf('/') !== -1) {
          c.image = raw + c.image.split('/').pop();
        }
      });
      LUMORA_DATA.articles.forEach(a => {
        if (a.image && a.image.indexOf('/') !== -1) {
          a.image = raw + a.image.split('/').pop();
        }
      });
    }
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', updateDataImagePaths);
  } else {
    updateDataImagePaths();
  }
}
